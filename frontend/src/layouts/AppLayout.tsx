...
const AppLayout = () => {
  ...
  return (
    <div data-theme="light" className="landing-shell drawer lg:drawer-open">
        ...
        <header className="sticky top-0 z-30 border-b border-slate-200/80 bg-white/88 backdrop-blur-xl">
          <div className="mx-auto flex w-full max-w-[1600px] items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
            <div className="navbar-start flex items-center gap-3">
              <label htmlFor="app-drawer" className="landing-button-secondary btn btn-square min-h-0 !h-11 !w-11 !rounded-2xl !p-0 lg:hidden">
            ...
            <div className="navbar-end flex items-center gap-3">
              {role === "admin" && (
                <Link to="/admin" className="landing-button-secondary min-h-0 rounded-2xl px-4 py-3 text-sm sm:text-base">
                  管理端
                </Link>
              )}
              {token && (
      ...
      <div className="drawer-side z-40">
        ...
        <aside className="flex min-h-full w-[290px] flex-col border-r border-slate-200/80 bg-white/90 px-4 py-5 text-slate-700 shadow-[0_24px_60px_rgba(15,23,42,0.08)] backdrop-blur-xl sm:w-[320px] lg:bg-white/72">
          <Link to="/app" onClick={closeDrawer} className="mb-5 flex items-center gap-3 rounded-[1.5rem] border border-slate-200/70 bg-white/86 px-4 py-4 shadow-[0_16px_40px_rgba(15,23,42,0.05)]">
            <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#6d6bf4] text-white shadow-[0_12px_30px_rgba(109,107,244,0.24)]">
              <BrandMark />
            </span>
            <div className="min-w-0">
              <div className="truncate text-lg font-bold tracking-[-0.03em] text-slate-900">{siteTitle}</div>
              <div className="text-xs font-medium uppercase tracking-[0.22em] text-slate-400">Workspace</div>
            </div>
          </Link>
